...
def main():
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        default="wikipedia_data/v6/hard/filtered",
        help="Path to save the output data",
    )
    parser.add_argument("--lang", type=str, help="Language code")
    parser.add_argument(
        "--threshold", type=float, default=0.4, help="Cosine similarity threshold"
    )
    parser.add_argument(
    ...
    ent_1 = [clean(str(x)) for x in res["Title"].values]
    ent_2 = [clean(str(x)) for x in res["English Title"].values]
