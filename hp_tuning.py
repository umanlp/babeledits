...
@hydra.main(config_path="configs", config_name="hp_tuning")
def main(cfg: DictConfig) -> None:
    ...
    yaml_dict["method"]["layers"] = int(yaml_dict["method"]["layers"][0]) # converting list to int for easier usage in wandb
    ...
    if cfg.log_subdir:
        log_dir = to_absolute_path(
            f"logs/{cfg.log_subdir}/{sweep_id}/{model_name}/{method}/{run_id}"
        )
    ...
    per_lang_scores = np.array(
        [
            summary["post"]["portability"][metric]
            for metric in summary["post"]["portability"]
            if "gloss" in metric
        ]
    )
    avg_score = np.mean(per_lang_scores)
    wandb.log({"xl_reliability_score": avg_score})

    print(f"XL Reliability Score in {cfg.tgt_langs}: {avg_score}")
    print(">>> FINISHED <<<")
