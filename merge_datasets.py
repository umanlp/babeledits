...
def uniform_sample_from_buckets(buckets, K, langs):
    ...
    for i in range(N):
        ...
        bucket = buckets[i]
        lang = langs[i]
        sampled_elements = sample_from_bucket(bucket, num_samples_from_bucket, global_sampled_elements, lang)
...
def sample_from_bucket(bucket, num_samples, global_sampled_elements, lang):
    ...
    sampled_elements = [(row["English Title"], lang) for _, row in sampled_elements.iterrows()]
    return sampled_elements

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--wiki_path", default="wikipedia_data/v4/processed", help="Path to the wiki file")
    parser.add_argument("--save_path", default="wikipedia_data/v4/filtered", help="Path to save the merged dataset")
    parser.add_argument("--num_samples", defualt=20000, type=int, help="Number of samples to draw")
    ...
    samples = uniform_sample_from_buckets(sorted_buckets, args.num_samples, sorted_langs)
