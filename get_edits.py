...
parser.add_argument(
    "--output_folder", default="datasets/v2/en_only", help="Output folder"
)
parser.add_argument(
    "--rel_path",
...
for lang in langs:
        ...
        for relation in relation_to_edges:
            for edge in relation_to_edges[relation]:
                if (
                    ...
                    edge["target_sense_src"] = clean(
                        target_senses[edge["target_id"]]["sense_src"]
                    )
                    edge["target_sense_en"] = clean(
                        target_senses[edge["target_id"]]["sense_en"]
                    )
            relation_to_edges[relation] = [
                edge
                for edge in relation_to_edges[relation]
                if "target_sense_en" in edge
            ]

    # Make it so that the key is a string (useful later)
    synset_to_senses = {
        str(synset.id): sense for synset, sense in synset_to_senses.items()
    }
    ...
    for synset in synset_to_relations:
        ...
        for relation in relation_to_edges:
            ...
            syn_pool = [
                ...
                if x["target_id"] not in target_synsets
            ]
    ...
    # Sampling a subset of edges for each relation
    synset_to_relations = {
        synset: {
            relation: edges
            for relation, edges in edge_list.items() if len(edges) > 0
        }
        for synset, edge_list in synset_to_relations.items()
    }
    ...
    output = {
        synset_id: {
            ...
            "relations": synset_to_relations[synset_id],
        }
    ...
    print(f"Time taken for lang {lang}: {(time.time()-t_start)/60} minutes")
